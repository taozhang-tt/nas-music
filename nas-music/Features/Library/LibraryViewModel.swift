...
final class LibraryViewModel: ObservableObject {
    ...
    func refresh() async {
        ...
        do {
            async let songsResult = provider.fetchSongs(offset: 0, limit: pageSize)
            async let albumsResult = provider.fetchAlbums(offset: 0, limit: pageSize)
            async let artistsResult = provider.fetchArtists(offset: 0, limit: pageSize)
            ...
            songsExhausted = loadedSongs.count < pageSize
            albumsExhausted = loadedAlbums.count < pageSize
            artistsExhausted = loadedArtists.count < pageSize

            state = (loadedSongs.isEmpty && loadedAlbums.isEmpty && loadedArtists.isEmpty) ? .empty : .loaded
        } catch {
    ...
    private func loadMoreSongs() async {
        ...
        do {
            let next = try await provider.fetchSongs(offset: songsOffset, limit: pageSize)
            songsOffset += next.count
            if next.count < pageSize { songsExhausted = true }
    ...
    private func loadMoreAlbums() async {
        ...
        do {
            let next = try await provider.fetchAlbums(offset: albumsOffset, limit: pageSize)
            albumsOffset += next.count
            if next.count < pageSize { albumsExhausted = true }
    ...
    private func loadMoreArtists() async {
        ...
        do {
            let next = try await provider.fetchArtists(offset: artistsOffset, limit: pageSize)
            artistsOffset += next.count
            if next.count < pageSize { artistsExhausted = true }
